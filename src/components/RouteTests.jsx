...
export default function RouteTests({}) {
  ...
  if (fileList) {
    ...
    if (nextOffset < fileList.length)
      nextIEC = fileList[nextOffset];
    // If it's too big, nextIEC is left null
    if (previousOffset >= 0)
      previousIEC = fileList[previousOffset];
  }
  ...
  return (
    <div id="testtwo-component">
      ...
      <p>Offset: {offset}</p>
      { nextIEC &&
        <p><Link to={`/test/${vr}/${nextIEC}`} className="button">Next IEC</Link></p>
      }
      { previousIEC &&
        <p><Link to={`/test/${vr}/${previousIEC}`} className="button">Previous IEC</Link></p>
      }
      <p><Link to={`/test/${vr}`} className="button">Go back to the VR page</Link></p>

      <p>Current value of A: {a}</p>
      <a href="#" className="button" onClick={() => { console.log(a); setA(a + 1)}}>Inc A</a>
